...
public class StudyTasksController extends HttpServlet {
    ...
    private void listTasks(HttpServletRequest req, HttpServletResponse resp) throws ModelException, ServletException, IOException {
        StudyTaskDAO dao = DAOFactory.createDAO(StudyTaskDAO.class);
        List<StudyTask> tasks = dao.listAll();
        req.setAttribute("tasks", tasks);
        req.getRequestDispatcher("/study-tasks.jsp").forward(req, resp);
    }

    private void showForm(HttpServletRequest req, HttpServletResponse resp, StudyTask task) throws ModelException, ServletException, IOException {
        UserDAO userDAO = DAOFactory.createDAO(UserDAO.class);
        req.setAttribute("task", task);
        req.setAttribute("users", userDAO.listAll());
        req.getRequestDispatcher("/form-study-task.jsp").forward(req, resp);
    }
}