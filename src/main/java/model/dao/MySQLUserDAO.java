...
public class MySQLUserDAO implements UserDAO {
	...
	public boolean save(User user) throws ModelException {
		...
		String sqlInsert = "INSERT INTO users (id, nome, sexo, email, senha) VALUES (DEFAULT, ?, ?, ?, ?);";
		
		db.prepareStatement(sqlInsert);
		db.setString(1, user.getName());
		db.setString(2, user.getGender());
		db.setString(3, user.getEmail());
		db.setString(4, user.getSenha());
	...
	@Override
	public List<User> findByName(String name) throws ModelException {
		DBHandler db = new DBHandler();
		List<User> users = new ArrayList<User>();
		
		// Pesquisa filtrando por partes do nome
		String sqlQuery = "SELECT * FROM users WHERE nome LIKE ?";
